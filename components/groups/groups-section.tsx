  ...
  return (
    <Card className="surface-card">
      ...
      <CardContent className="space-y-4">
        {showForm && (
          <form onSubmit={createGroup} className="grid gap-3 sm:grid-cols-2 border-t border-strivo-line pt-4">
            <div>
            ...
            <div className="sm:col-span-2">
              <Label>Goal</Label>
              <select
                className="w-full rounded-lg border border-strivo-line px-3 py-2 text-sm"
                value={form.goal}
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          {groups.map((g) => (
            <Link
              key={g.id}
              href={`/groups/${g.id}`}
              className="rounded-xl border border-strivo-line p-4 hover:border-primary/40 transition-colors"
            >
